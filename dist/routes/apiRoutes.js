...
} = require("../controllers/roomStatsController");
const {
  getGroupController,
  setGroupController
...
router.post("/remainingRequest", verifyToken, async (req, res) => {
  try {
    ...
    let [remainingRequest] = await Promise.all(services.map(service => service.catch(err => {
      console.log(err);
      return {
        ok: false,
        info: err
...
router.post("/playerData", verifyToken, async (req, res) => {
  try {
    ...
    let [newPlayerStats] = await Promise.all(services.map(service => service.catch(err => {
      console.log(err);
      return {
        ok: false,
        info: err
...
router.post("/playerDataFiltered/:playerName", verifyToken, async (req, res) => {
  try {
    ...
    let [playerSetFilters] = await Promise.all(services.map(service => service.catch(err => {
      return {
        ok: false,
        info: err
      };
...
router.post("/setPlayerData", verifyToken, async (req, res) => {
  try {
    ...
    let [newPlayerResult] = await Promise.all(services.map(service => service.catch(err => {
      console.log(error);
      return {
        ok: false,
        info: err
...
router.get("/getPlayers", verifyToken, async (req, res) => {
  try {
    ...
    let [playerData] = await Promise.all(services.map(service => service.catch(err => {
      console.log(err);
      return {
        ok: false,
        info: err
...
router.get("/getRoomStats", verifyToken, async (req, res) => {
  try {
    ...
    let [roomData] = await Promise.all(services.map(service => service.catch(err => {
      console.log(error);
      return {
        ok: false,
        info: err
...
router.post("/setRoomStats", verifyToken, async (req, res) => {
  try {
    ...
    let [newRoomStats] = await Promise.all(services.map(service => service.catch(err => {
      console.log(error);
      return {
        ok: false,
        info: err
...
router.get("/getGroups", verifyToken, async (req, res) => {
  try {
    ...
    let [groups] = await Promise.all(services.map(service => service.catch(err => {
      return {
        ok: false,
        info: err
      };
...
router.post("/setGroup", verifyToken, async (req, res) => {
  try {
    ...
    let [newGroup] = await Promise.all(services.map(service => service.catch(err => {
      console.log(error);
      return {
        ok: false,
        info: err
...
router.post("/getDefaultGroupFiltersData", verifyToken, async (req, res) => {
  try {
    ...
    let [groupData] = await Promise.all(services.map(service => service.catch(err => {
      return {
        ok: false,
        info: err
      };
...
router.post("/getTournamentsData", verifyToken, async (req, res) => {
  try {
    ...
    let [tournamentData] = await Promise.all(services.map(service => service.catch(err => {
      return {
        ok: false,
        info: err
      };
...
router.post("/admindashboard/users", async (req, res) => {
  ...
  try {
    ...
    let [userData] = await Promise.all(services.map(service => service.catch(err => {
      console.log(err);
      return {
        ok: false,
        info: err
